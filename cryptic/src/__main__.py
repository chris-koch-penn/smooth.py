...
from modules.org.transcrypt import compiler
from modules.org.transcrypt import utils
import os
import sys
import traceback
import site
...
transpilerDir = os.path.dirname(os.path.abspath(__file__)) .replace(
    '\\', '/')   # Set of Transcrypt itself
# Set dir of Transcrypt-only modules
...
# !!! Generalize to all platforms and Python versions
lowerSkipExtensions = ('.zip', '/dlls', '/lib', '/python37')
...
exitCodeNames = ('exitSuccess', 'exitCommandArgsError', 'exitNoLicense', 'exitSourceNotGiven',
                 'exitCannotRunSource', 'exitSpecificCompileError', 'exitGeneralCompileError')
...
def main():
    ...
    try:
        ...
        if not utils.commandArgs.source:
            ...
            return setExitCode(exitSourceNotGiven)

        if utils.commandArgs.source.endswith('.py') or utils.commandArgs.source.endswith('.js'):
            # Ignore extension
            utils.commandArgs.source = utils.commandArgs.source[:-3]
        ...
        if '/' in sourcePath:                                                   # If directory specified
            # Use it as source directory
            sourceDir = sourcePath.rsplit('/', 1)[0]
        else:                                                                   # Else
        ...
        if utils.commandArgs.run:
            try:
                ...
                utils.log(True, 'Error: cannot run {} using CPython: {}\n'.format(
                    utils.commandArgs.source, str(exception) .replace(' (<string>', '') .replace(')', '')))
