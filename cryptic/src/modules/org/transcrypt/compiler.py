...
from . import utils, sourcemaps, minify, static_check, type_check

inIf = False
ecom = True
noecom = False

dataClassDefaultArgTuple = (['init', True], ['repr', True], ['eq', True], [
...
class Program:
    def __init__(
        self,
        # All possible roots of the module path, the latter uniquely denoted by the dotted module name
        moduleSearchDirs,
        # Set of symbols either passed on the command line, deduced from command line switches etc.
        symbols,
        ...
        self.envir = envir
        self.javascriptVersion = int(
            utils.commandArgs.esv) if utils.commandArgs.esv else 6

        self.moduleDict = {}    # Administration of all modules that play a role in this program
        # Pending imports, enables showing load sequence in case a module cannot be loaded
        self.importStack = []

        # Set paths
        self.sourcePrepath = os.path.abspath(
            utils.commandArgs.source) .replace('\\', '/')
        ...
        self.optionsChanged = utils.commandArgs.projectOptions != project.get(
            'options')
        ...
        try:
            ...
            # Report only failure of searching runtime, so clear any history
            ...
            # Report only failure of searching for main, so clear any history
            self.searchedModulePaths = []
...
class Module:
    def __init__(self, program, name, __name__, filter):
        ...
        if (
            ...
            or
            not os.path.isfile(self.targetPath) or os.path.getmtime(
                self.sourcePath) > os.path.getmtime(self.targetPath)
        ):
            ...
            if not utils.commandArgs.nomin:
                utils.log(
                    True, 'Saving minified target code in: {}\n', self.targetPath)
                minify.run(
                    self.program.targetDir,
            ...
            if not javascriptDigest:
                ...
                self.prettyTargetCode = open(
                    self.prettyTargetPath, 'r') .read()
    ...
    def findPaths(self, filter):
        ...
        relSourceSlug = filter(
            rawRelSourceSlug) if filter and utils.commandArgs.alimod else rawRelSourceSlug
        ...
        for searchDir in self.program.moduleSearchDirs:
            ...
            if os.path.isfile(self.pythonSourcePath) or os.path.isfile(self.javascriptSourcePath):
                ...
                self.isJavascriptOnly = os.path.isfile(
                    self.javascriptSourcePath) and not os.path.isfile(self.pythonSourcePath)
            ...
            self.program.searchedModulePaths.extend(
                [self.pythonSourcePath, self.javascriptSourcePath])
        else:
    ...
    def generateJavascriptAndPrettyMap(self):
        ...
        if utils.commandArgs.map or utils.commandArgs.anno:
            ...
            instrumentedTargetLines = ''.join(
                self.generator.targetFragments) .split('\n')
            ...
            for targetLine in instrumentedTargetLines:
                ...
                # Take the appended line number, e.g. the string '000014'
                sourceLineNrString = targetLine[-sourcemaps.lineNrLength:]
                # Turn it into an integer, e.g. 14
                sourceLineNr = int('1' + sourceLineNrString) - \
                    sourcemaps.maxNrOfSourceLinesPerModule
                # Obtain non-instrumented line by removing the appended line number
                targetLine = targetLine[: -sourcemaps.lineNrLength]

                # Only append non-emptpy statements and their number info
                # If the non-instrumented line isn't empty
                if targetLine.strip() != ';':
                    # If annotations comments have to be prepended
                    if self.generator.allowDebugMap:
                        # Prepend them
                        targetLine = '/* {} */ {}'.format(
                            sourceLineNrString, targetLine)
                    # Add the target line, with or without prepended annotation comment
                    targetLines.append(targetLine)

                    # Store line nrs for source map
                    if utils.commandArgs.map:
                        # Remember its line number to be able to generate a sourcemap
            ...
            targetLines = [line for line in ''.join(
                self.generator.targetFragments) .split('\n') if line.strip() != ';']
    ...
    def parse(self):
        def pragmasFromComments(sourceCode):
            ...
            tokens = tokenize.tokenize(io.BytesIO(
                sourceCode.encode('utf-8')) .readline)
                ...
                if tokenType == tokenize.COMMENT:
                    ...
                    if strippedComment.startswith('__pragma__'):
                       ...
                       # Remember line index of multi-line pragma, like: # __pragma__ (...
                        ...
                        shortPragmaCommentLineIndices.append(
                            startRowColumn[0] - 1)
            ...
            for pragmaCommentLineIndex in pragmaCommentLineIndices:
                ...
                if "('ecom')" in pragma or '("ecom")' in pragma:
                    ...
                    sourceLines[pragmaCommentLineIndex] = indentation + \
                        tail.lstrip()
            ...
            for shortPragmaCommentLineIndex in shortPragmaCommentLineIndices:
                head, tail = sourceLines[shortPragmaCommentLineIndex] .rsplit(
                    '#', 1)
                ...
                if pragmaName == 'ecom':
                    ...
                    sourceLines[shortPragmaCommentLineIndex] = '{}__pragma__ (\'{}\'); {}; __pragma__ (\'{}\')' .format(
                        indent, pragmaName, head, pragmaName[2:])    # Correct!
                else:
            ...
            for sourceLine in sourceLines:
                if sourceLine == ecom:
                    ...
                    if not lStrippedSourceLine[:4] in {"'''?", "?'''", '"""?', '?"""'}:
                        uncommentedSourceLines.append(sourceLine.replace(
                            '#?', '', 1) if lStrippedSourceLine.startswith('#?') else sourceLine)
                else:
...
class Generator (ast.NodeVisitor):
    ...
    def __init__(self, module):
        ...
        self.aliases = [
            ...
            ('switch', 'py_switch'),
            # Only for the type metaclass, the type operator is dealt with separately in visit_Call
        ...
        self.stubsName = 'org.{}.stubs.'.format(
            self.module.program.envir.transpiler_name)
        ...
        self.operators = {
            ...
            # Used only for JavaScript %, which differs from Python %
            ast.Mod: ('%', 14),
        ...
        self.allowDebugMap = utils.commandArgs.anno and not self.module.sourcePath.endswith(
            '.js')
        ...
        # For speed, tuples are translated to bare JavaScript arrays if they're just indices. Will autoreset.
        self.stripTuple = False
        # For correctness, tuples are translated to bare JavaScript arrays if they are assignment target in a JavaScript 6 for-loop. Will not autoreset.
        self.stripTuples = False
        self.replaceSend = False

        try:
            self.visit(module.parseTree)
            # Last target fragment doesn't have a '\n' to replace in the emit method
        ...
        if self.tempIndices:
            raise utils.Error(
                message='\n\tTemporary variables leak in code generator: {}'.format(
                    self.tempIndices)
            )
    ...
    # Convention: only called at emission time or file name fabrication time
    ...
    def emit(self, fragment, *formatter):
        ...
        self.targetFragments.insert(self.fragmentIndex, fragment.format(
            *formatter) .replace('\n', self.lineNrString + '\n'))
        self.fragmentIndex += 1
    ...
    def adaptLineNrString(self, node=None, offset=0):
        ...
        if utils.commandArgs.map or utils.commandArgs.anno:
            ...
            self.lineNrString = str(
                sourcemaps.maxNrOfSourceLinesPerModule + lineNr)[1:]
    ...
    def emitSubscriptAssign(self, target, value, emitPathIndices=lambda: None):
        if type(target.slice) == ast.Index:        # Always overloaded
            if type(target.slice.value) == ast.Tuple:
                ...
                # Free function tries .__setitem__ (overload) and [] (native)
            ...
            if self.allowOperatorOverloading:
                # Free function tries .__setitem__ (overload) and .__setslice__ (native)
            ...
            if target.slice.step:
                ...
                # Must be null rather than 1, see Array.prototype.__setslice__
                self.emit('null')
            self.emit(', ')
            ...
            self.visit(target.value)
            # Method, since extended slice access is always overloaded
            self.emit('.__setitem__ (')
            self.emit('[')
    ...
    def useModule(self, name):
        # Remember line nr of import statement for the error report
        self.module.program.importStack[-1][1] = self.lineNr
        # Must be done first because it can generate a healthy exception
    ...
    def visit_arguments(self, node):   # Visited for def's, not for calls
        ...
        for arg, expr in reversed(list(zip(reversed(node.args), reversed(node.defaults)))):
            if expr:
                ...
                self.emit(
                    'if (typeof {0} == \'undefined\' || ({0} != null && {0}.hasOwnProperty ("__kwargtrans__"))) {{;\n', self.filterId(arg.arg))
        ...
        if self.allowKeywordArgs:
            ...
            self.emit(
                'if (arguments [{0}] && arguments [{0}].hasOwnProperty ("__kwargtrans__")) {{\n', self.getTemp('ilastarg'))
            self.indent()
            self.emit(
    ...
    def visit_Assign(self, node):
        ...
        def assignTarget(target, value, pathIndices=[]):
            ...
            if type(target) == ast.Subscript:
                ...
                if isPropertyAssign and target.id != self.getTemp('left'):
                    self.emit('Object.defineProperty ({}, \'{}\', '.format(
                        self.getScope() .node.name, target.id))
                    self.visit(value)
                    emitPathIndices()
                    self.emit(')')
                else:
                    if type(target) == ast.Name:
                        if type(self.getScope() .node) == ast.ClassDef and target.id != self.getTemp('left'):
                            # The target is a class attribute
                            self.emit('{}.'.format(
                                '.'.join([scope.node.name for scope in self.getAdjacentClassScopes()])))
        ...
        def walkTarget(expr, pathIndices):
            if type(expr) in targetLeafs:          # It's an LValue, matching an RHS leaf source
                ...
                # Add indexing level for that sequence
                pathIndices.append(None)
                for index, elt in enumerate(expr.elts):
                    pathIndices[-1] = index
                    # Walk deeper until finally pathIndices is used in emitting an RHS leaf
                    walkTarget(elt, pathIndices)
                # Remove the indexing level since we're done with that sequence
                pathIndices.pop()
        ...
        def getIsPropertyAssign(value):
            if self.isCall(value, 'property'):
                ...
                # Assume it's a tuple or a list of properties (and so recursively)
                try:
        ...
        isPropertyAssign = type(
            self.getScope() .node) == ast.ClassDef and getIsPropertyAssign(node.value)
    ...
    def visit_AugAssign(self, node):
        if self.allowOperatorOverloading:
            ...
            self.expectingNonOverloadedLhsIndex = True
            # No need to emit var first, it has to exist already
            self.visit(node.target)
    ...
    def visit_Call(self, node, dataClassArgDict=None):
        ...
        def emitKwargTrans():
            ...
            if hasSeparateKeyArgs:
                if hasKwargs:
                    ...
                    # Only if not terminated already because hasKwargs
                    self.emit('}}')
                    ...
                    if os.path.isfile(filePath):
                        ...
                        if fileName.endswith('.js'):
                            ...
                            includedCode = utils.digestJavascript(
                                includedCode, self.module.program.symbols, not utils.commandArgs.dnostrip or utils.commandArgs.anno, self.allowDebugMap) .digestedCode
                        return includedCode
                    else:
        ...
        if type(node.func) == ast.Name:
            ...
            if node.func.id == 'type':
                # Don't use general alias, since this is the type operator, not the type metaclass
                self.emit('py_typeof (')
            ...
            # property () factory function
                ...
                self.emit('{0}.call ({1}, {1}.{2}'.format(node.func.id, self.getScope(
                    ast.ClassDef) .node.name, self.filterId(node.args[0].id)))
                if len(node.args) > 1:
                    self.emit(', {}.{}'.format(self.getScope(
                        ast.ClassDef) .node.name, node.args[1].id))
                self.emit(')')
                return
            ...
            # globals () function
                ...
                # y18m08d23 Variable __all__ created as a dict itself
                self.emit('__all__')
                return
            # __pragma__'s in many varieties, syntactically calls, but semantically compile time directives
                ...
                # Automatic conversion to iterable supported
                elif node.args[0] .s == 'iconv':
                    self.allowConversionToIterable = True
                # Automatic conversion to iterable not supported
                elif node.args[0] .s == 'noiconv':
                    self.allowConversionToIterable = False

                # Translate for ... in ... : ... literally to for (... in ...) {...},
                elif node.args[0] .s == 'jsiter':
                    # to enable iterating JavaScript objects that are not dicts
                    self.allowJavaScriptIter = True
                # Dictionary keys without quotes are identifiers
                elif node.args[0] .s == 'nojsiter':
                    self.allowJavaScriptIter = False

                # Python calls get compiled to direct JavaScript calls
                elif node.args[0] .s == 'jscall':
                    self.allowJavaScriptCall = True
                # Python calls get compiled to calls via a JavaScript property, allowing bound method assignment
                elif node.args[0] .s == 'nojscall':
                    self.allowJavaScriptCall = False

                # Dictionary keys without quotes are string literals
                ...
                # Nonexistent dict keys or list indices generate an exception
                elif node.args[0] .s == 'keycheck':
                    self.allowKeyCheck = True
                # Nonexistent dict keys or list indices produce undefined values
                ...
                # Automatic conversion to truth value supported
                elif node.args[0] .s == 'tconv':
                    self.allowConversionToTruthValue = True
                # Automatic conversion to truth value not supported
                ...
                # Include JavaScript code literally in the output
                    ...
                    try:
                        try:
                            code = node.args[1] .s.format(* [
                                eval(
                                    compile(
                                        # Code to compile (can be AST or source)
                                        ast.Expression(arg),
                                        '<string>',             # Not read from a file
                                        # Code is an expression, namely __include__  (<fileName>) in most cases
                                        'eval'
                                    ),
                                    {},
                                    {'__include__': include}
                ...
                # Include code transpiled by external process in the output
                ...
                # Allow use of the 'globals' function for this whole module
                elif node.args[0] .s == 'xglobs':
                    self.allowGlobals = True
                # Disallow use of the 'globals' funciton for this whole module
                elif node.args[0] .s == 'noxglobs':
                    self.allowGlobals = False

                # Start emitting kwargs code for FunctionDef's
                elif node.args[0] .s == 'kwargs':
                    self.allowKeywordArgs = True
                # Stop emitting kwargs code for FunctionDef's
                elif node.args[0] .s == 'nokwargs':
                    self.allowKeywordArgs = False

                # Overloading of a small sane subset of operators allowed
                elif node.args[0] .s == 'opov':
                    self.allowOperatorOverloading = True
                # Operloading of a small sane subset of operators disallowed
                    ...
                    # Easier dealt with on statement / expression level in self.visit
                    pass

                elif node.args[0] .s == 'xpath':
                    self.module.program.moduleSearchDirs[1: 1] = [
                        elt.s for elt in node.args[1] .elts]

                else:
                    raise utils.Error(
                        lineNr=self.lineNr,
                        message='\n\tUnknown pragma: {}'.format(
                            node.args[0] .s if type(
                                node.args[0]) == ast.Str else node.args[0]
                        )
                    )
                return
        ...
        # send () call
            ...
            # Construct Attribute instead of bare Call node on the fly and visit it
            node.func.attr == 'send'
        ):
            # Encapsulate in function to prevent minifier complaining if value isn't used
            ...
            self.emit('}}) ()')
            # The newly created node was visited by a recursive call to visit_Call. This replaces the current visit.
            return
                    ...
                    ast.Call(
                        func=ast.Call(
                            ...
                            args=[
                                ast.Name(
                                    id='.'.join(
                                        [scope.node.name for scope in self.getAdjacentClassScopes(True)]),
                                    ctx=ast.Load
                                ),
                                ast.Str(
        ...
        if dataClassArgDict != None:
            ...
            for index, expr in enumerate(node.args):
                ...
                if value != None:
                    ...
                    raise utils.Error(
                        message='Arguments to @dataclass can only be constants True or False')
    ...
    def visit_ClassDef(self, node):
        ...
        if node.decorator_list:
            if type(node.decorator_list[-1]) == ast.Name and node.decorator_list[-1] .id == 'dataclass':
                isDataClass = True
                # Use default decorator args (or rather there are no args)
                ...
                isDataClass = True
                dataClassArgDict = {}
                # Adapt dataClassArgDict to decorator args
        ...
        if node.decorator_list:
            self.emit(' ')
            if self.allowOperatorOverloading:
                # The decorator is called, it may be a ast.Name (paramless) of a function or the result of an ast.Call (with params)
                self.emit('__call__ (')

            for decorator in node.decorator_list:
                if decoratorsUsed > 0:
                    self.emit(' (')
                # This can either be a ast.Name (paramless dec) or and ast.Call (dec with params)
        ...
        # LHS is simple name, class var assignment generates initialisation of field in object literal
        inlineAssigns = []
        propertyAssigns = []    # LHS is simple name, RHS is property constructor call
        # Dataclass instance var assignment, these are also the params to the generated __init__
        initAssigns = []
        ...
        # Representation consists of instance vars, class vars and properties, in the order encountered
        reprAssigns = []
        ...
        # Comparing is done by instance vars and properties, in the order encountered
        compareAssigns = []
        ...
        for statement in node.body:
            if self.isCommentString(statement):
                ...
                if type(statement.value) == ast.Call and type(statement.value.func) == ast.Name and statement.value.func.id == 'property':
                    ...
                    # For defaults a class var will do
                    inlineAssigns.append(statement)
        ...
        if node.keywords:
            if node.keywords[0] .arg == 'metaclass':
                ...
                raise utils.Error(
                    lineNr=self.lineNr,
                    message='\n\tUnknown keyword argument {} definition of class {}'.format(
                        node.keywords[0] .arg, node.name)
                )
...
return list (selfFields).''' + comparatorName + '''(list (otherFields));
        ...
        self.mergeList.append(utils.Any(
            className='.'.join(
                [scope.node.name for scope in self.getAdjacentClassScopes()]),
        ...
        def emitMerges():
            def emitMerge(merge):
                ...
                if merge.isDataClass:
                    ...
                    self.emit(';\n__mergefields__ ({}, {{',
                              self.filterId(merge.className))
                    self.emit('__reprfields__: new Set ([{}]), ', ', '.join(
                        '\'{}\''.format(reprAssign.target.id) for reprAssign in merge.reprAssigns))
                    self.emit('__comparefields__: new Set ([{}]), ', ', '.join('\'{}\''.format(
                        compareAssign.target.id) for compareAssign in merge.compareAssigns))
                    self.emit('__initfields__: new Set ([{}])', ', '.join(
                        '\'{}\''.format(initAssign.target.id) for initAssign in merge.initAssigns))
                    self.emit('}})')
        ...
        def emitProperties():
            def emitProperty(className, propertyName, getterName, setterName=None):
                ...
                if setterName:
                    self.emit('property.call ({0}, {0}.{1}, {0}.{2})'.format(
                        className, getterName, setterName))
                else:
                    self.emit('property.call ({0}, {0}.{1})'.format(
            ...
            while self.propertyAccessorList:
                ...
                for propertyAccessor2 in self.propertyAccessorList:
                    ...
                    if className == className2 and propertyName == propertyName2 and isGetter != isGetter2:
                        ...
                        if isGetter:
                            emitProperty(className, propertyName,
    ...
    # Currently dict element only, rest can be done with empty slice assignment
    ...
    def visit_Dict(self, node):
        # If we don't want JavaScript treatment of keys, for literal keys it doesn't make a difference
        if not self.allowJavaScriptKeys:
            for key in node.keys:
                # but if there's only one non-literal key there's a difference, and all keys are treated the Python way
                if not type(key) in (ast.Str, ast.Num):
                    self.emit('dict ([')
                    for index, (key, value) in enumerate(zip(node.keys, node.values)):
                        self.emitComma(index)
                        self.emit('[')
                        # In a JavaScript list, name is evaluated as variable or function call to produce a key
                        self.visit(key)
                        self.emit(', ')
                        self.visit(value)
                        self.emit(']')
        ...
        if self.allowJavaScriptIter:
            ...
            # Since we didn't return, we want identifier keys to be treated as string literals
            self.emit('dict ({{')
        for index, (key, value) in enumerate(zip(node.keys, node.values)):
            self.emitComma(index)
            # The key may be a string or an identifier, the latter normally would be filtered, which we don't want
            self.idFiltering = False
            # In a JavaScript object literal, an identifier isn't evaluated but literally taken to be a key.
            self.visit(key)
            self.idFiltering = True
            self.emit(': ')
            self.visit(value)
    ...
    def visit_For(self, node):
        ...
        optimize = (
            # Since 'var' is emitted, target must not yet exist, so e.g. not be element of array
            type(node.target) == ast.Name and
        ...
        if self.allowJavaScriptIter:
            ...
            self.visit(node.target)
            self.emit(' = ')
            self.visit(node.iter.args[0] if len(
                node.iter.args) > 1 else ast.Num(0))
            self.emit('; ')
            self.visit(node.target)
            self.emit(' < ' if step > 0 else ' > ')
            self.visit(node.iter.args[1] if len(
                node.iter.args) > 1 else node.iter.args[0])
            self.emit('; ')
            self.visit(node.target)
            if step == 1:
                self.emit('++')
        ...
        if node.orelse:
            # One off, since 'else' doesn't have it's own node and line nr
    ...
    def visit_FunctionDef(self, node, anAsync=False):
        ...
        if not nodeName == '__pragma__':    # Don't generate code for the dummy pragma definition starting the extraLines in utils
            ...
            isMethod = not (isGlobal or type(self.getScope() .node) in (
                ast.FunctionDef, ast.AsyncFunctionDef))  # Global or function scope, so it's no method
            ...
            if decorate:
                if isMethod:
                    if jsCall:
                        ...
                        self.emit('get {} () {{return {} (this, ',
                                  self.filterId(nodeName), getter)
                elif isGlobal:
                ...
                if isMethod:
                    if jsCall:
                        self.emit('{}: function', self.filterId(
                            nodeName), 'async ' if anAsync else '')
                    else:
                        if isStaticMethod:
                            self.emit('get {} () {{return {}function', self.filterId(
                                nodeName), 'async ' if anAsync else '')
                        else:
                            self.emit('get {} () {{return {} (this, {}function', self.filterId(
                                nodeName), getter, 'async ' if anAsync else '')
                elif isGlobal:
                    if type(node.parentNode) == ast.Module and not nodeName in self.allOwnNames:
                        self.emit('export ')
                    self.emit('var {} = {}function', self.filterId(
                        nodeName), 'async ' if anAsync else '')
                else:
                    self.emit('var {} = {}function', self.filterId(
                        nodeName), 'async ' if anAsync else '')
            ...
            skipFirstArg = jsCall and not (
                not isMethod or isStaticMethod or isProperty)
            ...
            if skipFirstArg:
                ...
                if isClassMethod:
                    self.emit(
                        'var {} = \'__class__\' in this ? this.__class__ : this;\n', firstArg)
                else:
                    self.emit('var {} = this;\n', firstArg)
    ...
    def visit_Global(self, node):
        self.getScope(ast.FunctionDef,
                      ast.AsyncFunctionDef) .nonlocals.update(node.names)
    ...
    def revisit_Import(self, importHoistMemo):     # Import ... can only import modules
        ...
        names = [alias for alias in node.names if not alias.name.startswith(
            self.stubsName)]
        ...
        for index, alias in enumerate(names):
            ...
            if alias.asname and not alias.asname in (self.allOwnNames | self.allImportedNames):
                ...
                self.emit('__nest__ ({}, \'{}\', __module_{}__);\n', self.filterId(
                    head), self.filterId(tail), self.filterId(module.name .replace('.', '_')))
    ...
    # From ... import ... can import modules or facitities offered by modules
    def revisit_ImportFrom(self, importHoistMemo):
        ...
        try:
            ...
            # If none of the possibilities below succeeds, report all searched paths
            self.module.program.searchedModulePaths = []
            namePairs = []
            facilityImported = False
            for index, alias in enumerate(node.names):
                # * Never refers to modules, only to facilities in modules
                if alias.name == '*':
                    if len(node.names) > 1:
                        raise utils.Error(
                            lineNr=self.lineNr,
                    ...
                    # Try if alias denotes a module, in that case don't do the 'if namepairs' part
                    try:
                        # So, attempt to use alias as a module
                        module = self.useModule(
                            '{}.{}'.format(node.module, alias.name))
                        self.emit('import * as {} from \'{}\';\n', self.filterId(alias.asname) if alias.asname else self.filterId(
                            alias.name), module.importRelPath)  # Modules too can have asName
                        # Add import to allImportedNames of this module
                        self.allImportedNames.add(alias.asname or alias.name)
                    except:                                                                 # It's a facility rather than a module
                        module = self.useModule(node.module)
                        namePairs.append(
                            utils.Any(name=alias.name, asName=alias.asname))
                        facilityImported = True

            # At least one alias denoted a facility rather than a module
            if facilityImported:
                # Use module that contains it
                module = self.useModule(node.module)
                namePairs.append(
                    utils.Any(name=alias.name, asName=alias.asname))
            ...
            utils.enhanceException(
                exception,
                lineNr=self.lineNr,
                message='\n\tCan\'t import from module \'{}\''.format(
                    node.module)
            )

    def visit_JoinedStr(self, node):
        self.emit(repr(''.join(
            [value.s if type(value) == ast.Str else '{{}}' for value in node.values])))
        self.emit('.format (')
        index = 0
    ...
    def visit_Lambda(self, node):
        # Extra () needed to make it callable at definition time
    ...
    def visit_ListComp(self, node, isSet=False, isDict=False, isGenExp=False):
        ...
        self.indent()
        self.emit('var {} = [];\n', self.nextTemp('accu'))
        # Leave original in intact, just for neatness
        nestLoops(node.generators[:])
        self.emit(
            'return {}{}{};\n',
            'set (' if isSet else 'dict (' if isDict else '{} ('.format(
                self.filterId('iter')) if isGenExp else '',
            self.getTemp('accu'),
    ...
    def visit_Module(self, node):
        ...
        if self.allowDocAttribs:
            docString = ast.get_docstring(node)
            if docString:
                # Should be done before generation of exported names
                self.allOwnNames.add('__doc__')

        # Prepair to generate hoisted fragments near start of fragments
        # Subsequent emits will also hoist self.lineNr, subsequent revisits will even adapt self.lineNrString
        ...
        if self.allowGlobals:
            self.emit(
                # Has nothing to do with emitting an export list, just another importable (so exported) module level variable __all__
                'var __all__ = dict ({{'
                +
                ', '.join([
        ...
        if utils.commandArgs.xreex or self.module.sourcePrename == '__init__':
            if self.allImportedNames:
                self.emit('export {{{}}};\n', ', '.join([self.filterId(
                    importedName) for importedName in self.allImportedNames]))     # This emits an export list
    ...
    def visit_Num(self, node):
        self.emit('complex (0, {})'.format(node.n.imag) if type(
            node.n) == complex else '{}'.format(node.n))
    ...
    def visit_Str(self, node):
        # Use repr (node.s) as second, rather than first parameter, since node.s may contain {}
    ...
    def visit_Subscript(self, node):
        if type(node.slice) == ast.Index:
            # Always overloaded, it must be an RHS index
            if type(node.slice.value) == ast.Tuple:
                ...
                # Free function tries .__getitem__ (overload) and [] (native)
                self.emit('__getitem__ (')
                self.visit(node.value)
                self.emit(', ')
                self.visit(node.slice.value)
            ...
            if self.allowOperatorOverloading:
                # Free function, tries .__getitem__ (overload) and .__getslice__ (native)
    ...
    def visit_Try(self, node):
        ...
        # try ... finally (without any catch) is also valid Python
        if node.handlers:
            self.emit('catch ({}) {{\n', self.nextTemp('except'))
            self.indent()
    ...
    def visit_Tuple(self, node):
        # Tuples used as indices are stripped for speed
        keepTuple = not (self.stripTuple or self.stripTuples)
        # Tuples used as indices are stripped for speed, only strip first tuple encountered
        self.stripTuple = False
    ...
    def visit_With(self, node):
        ...
        for item in node.items:
            # Should be in surrounding scope but may be overwritten, so use var rather than let
            self.emit('var ')
    ...
    def visit_Yield(self, node):
        self.getScope(ast.FunctionDef,
                      ast.AsyncFunctionDef) .containsYield = True
        self.emit('yield')
        if (node.value != None):
            self.emit(' ')
            self.visit(node.value)
