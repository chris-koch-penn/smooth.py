...
sys.path.append(
    '{}/pyflakes'.format(os.path.dirname(os.path.abspath(__file__)) .replace('\\', '/')))
...
def run(sourceFile, parseTree):
    ...
    utils.log(messageHolders or utils.commandArgs.verbose,
              'Performing lightweight consistency check on module: {}'.format(sourceFile))
    if messageHolders:
